...
void Notes::ValidateFile(const size_t amount_notes) {
    if (notes_.empty()) {
        std::cout << "Ошибка! Заметки отсутствуют. Создайте заметку."
                  << std::endl;
...
// Загрузка заметок из файла. Если файл открывается, то происходит чтение
// содержимого файла построчно. Если файл не удается открыть, то выводится
// сообщение об ошибке.
...
void Notes::DelNote() {
    if (CheckAmountNotes() && read_file_) {
        ...
        if (CheckNoteNum(note_num)) {
            ...
            std::cout << "Заметка " << note_num << " успешно удалена."
                      << std::endl;
            std::cout << std::endl;
        }
    }
...
// сообщение об ошибке.
// Первая цифра в файле - количество заметок в векторе notes_.
